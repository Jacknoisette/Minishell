...
/*   Updated: 2025/02/19 15:35:33 by jdhallen         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	pipe_case(t_info *info)
{
	...
	if (act_is_bash_op(info->main_lst, &info->cmd_list, &info->tmp, info->pos) == BREAK)
		return (BREAK);
	line = create_new_node_line(ft_itoa(cmd_list_count(&info->cmd_list)));
	list_add_back_line(info->line_list, line);
...
int	get_lst_cmd_core(t_info *info)
{
	...
	info->res = search_bash_op(info->main_lst, &info->cmd_list, &info->tmp, info->pos);
